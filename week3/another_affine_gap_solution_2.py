...
print("     " + "    ".join(virt_keys))
for i in range(len(score_BLOSUM)):
    val = ""
    for j in range(len(score_BLOSUM)):
        val +=  " " + str(score_BLOSUM[i][j]).rjust(4)
    print(virt_keys[i] + val)
...
def global_allignment_affine_gaps(s1,s2,sigma,epsilon):
	...
	s = [[[0 for i in range(m+1)] for j in range(n+1)] for k in range(3)]
	backtrack = [[[0 for i in range(m+1)] for j in range(n+1)] for k in range(3)]
