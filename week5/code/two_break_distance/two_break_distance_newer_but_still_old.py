...
def check_graph(graph):
    if len(graph) > 0:
        ...
        next_node_offset = 1 if prev % 2 == 1 else -1
        if prev + next_node_offset != start[0]:
            raise ValueError("idx: {0}. {1} needs to correspond with {2}".format("end-start", prev, start[0]))
...
def convert_cycles_strings_to_graphs(cycles_set):
    gen_as_ints = []
    for cycles in cycles_set:
...
if __name__ == '__main__':
    ...
    genomes,synteny_count = convert_cycles_strings_to_graphs(cycles_set)
